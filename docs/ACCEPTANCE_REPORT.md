...
## 6. Доработка после отчёта (T-056)

Единственное число приёмки, которое зависело от нашего кода, а не от машины и
не от внешних сторон, — полный путь ставки в процессе (47.7 мс при пороге 15).
Причина в отчёте названа: три обращения в PostgreSQL перед атомарным ядром.

Сделано: проверка допуска — пользователь, лот и задаток — сведена в один запрос
соединением. Кэша сознательно нет: у любого времени жизни кэша есть окно, в
котором ставку принимают у участника, чей задаток уже вернули.

Измерено A/B на одних данных, формы чередуются в одном процессе (300 прогонов
на форму): медиана проверки **1.81 → 1.01 мс**, p99 **2.77 → 1.50 мс**.

**Что этим НЕ доказано:** что полный путь под нагрузкой уложился в 15 мс.
Контрольный прогон QA-01 пришёлся на машину с 1.15 ГБ свободной памяти и просел
в шесть раз целиком — включая атомарное ядро в Redis, которого правка не
касается. Такой прогон не сравнивают, его выбрасывают; перезамер нужен на
спокойной машине или на stage.
