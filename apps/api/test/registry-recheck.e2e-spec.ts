...
beforeEach(async () => {
  ...
  /**
   * Сначала дожидаемся тишины в очереди, только потом чистим.
   *
   * Расписание BullMQ даёт первый тик сразу при подъёме процесса, и обход
   * находит лоты, оставшиеся от предыдущих файлов прогона. Его задачи пишут
   * в registry_checks — прямо посреди очистки, между удалением проверок и
   * удалением лотов. Результат: очистка спотыкается о внешний ключ, и падает
   * тест, который к реестру отношения не имеет.
   *
   * Это не флейк и не «показалось»: воспроизводится только в полном прогоне,
   * потому что в одиночку предыдущих лотов нет и обходить нечего.
   */
  await worker.drain();
