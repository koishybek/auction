...
export const envSchema = z
  ...
  .superRefine((env, ctx) => {
    /**
     * Ключ шифрования и ключ blind index обязаны различаться (CLAUDE.md §4.5).
     *
     * Проверка длины у каждого своя и по отдельности ничего об этом не говорит:
     * оператор, скопировавший один сгенерированный ключ в оба поля K8s-секрета,
     * получает штатно поднявшееся приложение с молча нарушенным инвариантом.
     * Цена ошибки не в аккуратности: blind index — детерминированный HMAC по
     * ИИН, а ИИН это двенадцать цифр, то есть пространство перебираемое. При
     * совпадении ключей утечка индекса компрометирует не поиск, а сами ФИО, ИИН,
     * телефоны и адреса.
     */
    if (env.PII_ENCRYPTION_KEY === env.PII_BLIND_INDEX_KEY) {
      ctx.addIssue({
        code: 'custom',
        path: ['PII_BLIND_INDEX_KEY'],
        message:
          'PII_BLIND_INDEX_KEY совпадает с PII_ENCRYPTION_KEY. Ключи обязаны различаться: ' +
          'утечка ключа поиска не должна раскрывать сами персональные данные',
      });
    }
  });
