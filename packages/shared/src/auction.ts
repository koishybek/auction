...
export interface AuctionStateView {
  ...
  /**
   * Сколько осталось до конца паузы SLA Freeze. `null` — паузы нет.
   *
   * Нужно вернувшемуся после обрыва: событие `sla_freeze` получают только те,
   * кто был на связи в момент заморозки, а баннер «возобновление через N с»
   * строится на этой величине. Без неё участник видит остановленный таймер и
   * ни одного объяснения (QA-03, найдено T-054).
   */
  readonly resumeInMs: number | null;
  /**
   * Псевдоним победителя. `null` — торги ещё идут либо ставок не было.
   *
   * Нужен вернувшемуся после закрытия: событие `auction_finished` получают
   * только те, кто был в комнате в тот момент.
   */
