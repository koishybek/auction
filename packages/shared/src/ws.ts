...
export interface StateSnapshotEvent {
  ...
  readonly seq: number;
  /**
   * Сколько осталось до конца паузы SLA Freeze. Поля нет, если паузы нет.
   *
   * Событие `sla_freeze` получают только те, кто был на связи в момент
   * заморозки. Вернувшийся после обрыва узнаёт о паузе единственным способом —
   * из снимка; без этого поля он видел бы остановленный таймер и ни одного
   * объяснения (QA-03).
   */
  readonly resume_in_ms?: number;
  /**
   * Псевдоним победителя. Поля нет, пока торги идут или если ставок не было.
   *
   * Как и `resume_in_ms`, это ответ вернувшемуся: событие `auction_finished`
   * получили те, кто был в комнате в момент закрытия, а снимок — единственное,
   * что осталось остальным.
   */
  readonly winner_blind_id?: string;
  /**
 ...
 *
 * Отказ называет актуальную цену и сумму следующей ставки: проигравший в гонке
 * обязан узнать, на что ставить дальше, из самого отказа. Иначе он ждёт общего
 * `bid_updated` или переспрашивает снимок — а на сотне одновременных ставок это
 * девяносто девять лишних запросов ровно в тот момент, когда лот и без них
 * нагружен сильнее всего (T-052).
 *
 * Полей нет у отказов, при которых цена не менялась или не существует: нет
 * задатка, не пройдена верификация, по лоту нет торгов.
 */
export interface BidRejectedEvent {
  ...
  /** Цена на момент отказа, целые тенге. */
  readonly current_price_kzt?: number;
  /** Сумма, которую сервер примет следующей. Считает сервер, не клиент. */
  readonly next_price_kzt?: number;
  /** Номер последней принятой ставки — по нему клиент отбрасывает запоздавший отказ. */
  readonly seq?: number;
}
